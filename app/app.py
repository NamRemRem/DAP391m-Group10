...
def main():
    ...
    st.markdown("A simple student project to analyze and predict the quality of marketplace reviews.")
    ...
    model_choice = st.sidebar.selectbox("Choose Model", options=["Logistic Regression", "Random Forest"])
    ...
    with col1:
        ...
        fig_pie = px.pie(pie_data, values='Count', names='Status', color_discrete_sequence=['#2ecc71', '#e74c3c'])
    ...
    with col2:
        ...
        fig_bar = px.bar(rating_counts, x='Rating', y='Reviews', color='Reviews', color_continuous_scale='Viridis')
    ...
    fig_line = px.line(trend_data, x='date', y='Daily Reviews', title="New Reviews Submitted Per Day")
    ...
    st.table(samples[['text', 'rating', 'sentiment_score', 'helpfulness_score', 'predicted_helpful']].head(num_samples))
