...
def extract_advanced_features_batch(texts):
    ...
    for doc in nlp.pipe(texts, batch_size=200):
        ...
        results.append((noun_count / num_tokens, adj_count / num_tokens, ner_count, readability, comparative_count))
    return pd.DataFrame(results, columns=['noun_ratio', 'adj_ratio', 'ner_count', 'readability', 'comparative_count'])
